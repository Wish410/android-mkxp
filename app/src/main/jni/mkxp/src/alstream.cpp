	...
	bool tryRead(SDL_RWops &ops, const char *ext)
	{
		...
		*srcOps = ops;
		...
		try
		{
			...
			if (!strcmp(sig, "RIFF"))
			{
				char waveHeader[37] = { 0 };
				SDL_RWread(srcOps, waveHeader, 1, 36);
				SDL_RWseek(srcOps, 0, RW_SEEK_SET);
				char waveBPS[3] = { 0 };
				waveBPS[1] = waveHeader[36];
				waveBPS[2] = waveHeader[37];
				if (!strcmp(waveBPS, "16")){
			...
			char id3v1sig[3] = { 0 };
			SDL_RWread(srcOps, id3v1sig, 1, 2);
			SDL_RWseek(srcOps, 0, RW_SEEK_SET);
			if (!strcmp(id3v1sig, "؞"))
			{
				source = createSDLSource(*srcOps, ext, STREAM_BUF_SIZE, looped);
				return true;
			}
			...
			char id3v2sig[4] = { 0 };
			SDL_RWread(srcOps, id3v2sig, 1, 3);
			SDL_RWseek(srcOps, 0, RW_SEEK_SET);
			if (!strcmp(id3v2sig, "ID3"))
			{
				source = createSDLSource(*srcOps, ext, STREAM_BUF_SIZE, looped);
				return true;
			}
