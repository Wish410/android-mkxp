...
#define PO_DESC_ALL \
	...
	PO_DESC(forcedWidth, int, 0) \
	PO_DESC(forcedHeight, int, 0) \
...
static void setupScreenSize(Config &conf)
{
	...
	if (conf.forcedWidth <= 0)
		conf.forcedWidth = (conf.rgssVersion == 1 ? 640 : 544);
	
	if (conf.forcedHeight <= 0)
		conf.forcedHeight = (conf.rgssVersion == 1 ? 480 : 416);
}
