...
static void mriBindingInit()
{
	...
	if (rgssVer >= 3)
	{
		...
		rb_define_global_const("RGSS_VERSION", rb_str_new("3.0.1",strlen("3.0.1\0")));
	}
	else
	{
...
RB_METHOD(_kernelCaller)
{
	...
	VALUE args[] = { rb_str_new(":in `<main>'",strlen(":in `<main>'\0")), rb_str_new("",strlen("\0")) };
...
{
	//Replace SHIFT_JIS with UTF-8 in string
	try{
...
static void runRMXPScripts(BacktraceData &btData)
{
	...
	for (long i = 0; i < scriptCount; ++i)
	{
		...
		const char *decodedStr = decodeBuffer.c_str();
		rb_ary_store(script, 3, rb_str_new(decodedStr,strlen(decodedStr)));
	}
